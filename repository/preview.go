...
// PullPreviews fetches previews from a remote URL or local database.
// The prepareRequest callback is used to attach auth tokens and headers to HTTP requests.
func PullPreviews(tx *sqlx.Tx, remoteUrl string, previewHashes []string, callback func(int, int, string, string), prepareRequest PrepareRequest) error {
...
// PushPreviews sends previews to a remote URL or local database.
// The prepareRequest callback is used to attach auth tokens and headers to HTTP requests.
func PushPreviews(tx *sqlx.Tx, remoteUrl string, userId string, previewHashes []string, callback func(int, int, string, string), prepareRequest PrepareRequest) error {
	dataUrl := remoteUrl + "/previews"
	client := &http.Client{}
	totalPreviews := len(previewHashes)
	processedPreviews := 0
