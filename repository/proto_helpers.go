...
// ChunkInfo holds chunk metadata. Defined here to avoid a circular dependency
// with chunk_service. When chunk_service moves to core, both can share this type.
type ChunkInfo struct {
	Hash string `json:"hash"`
	Size int    `json:"size"`
}

// --- To Protobuf conversion helpers ---
...
func ToPbChunkInfos(chunkInfos []ChunkInfo) []*repositorypb.ChunkInfo {
...
// ToPbStatus converts a single Status model to protobuf.
...
// --- From Protobuf conversion helpers ---
...
func FromPbChunkInfo(pb *repositorypb.ChunkInfo) ChunkInfo {
	return ChunkInfo{
		Hash: pb.Hash,
		Size: int(pb.Size),
	}
...
func FromPbChunkInfos(pbs []*repositorypb.ChunkInfo) []ChunkInfo {
	chunkInfos := make([]ChunkInfo, len(pbs))
	for i, pb := range pbs {
